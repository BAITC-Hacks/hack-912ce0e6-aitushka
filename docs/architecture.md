...
flowchart TD
    ...
    B --> API
    API --> AC["ai_context.py: выбранные факты и псевдонимы"]
    AC --> AI["ai_service.py: лимиты, кеш, проверка ответа"]
    AI --> O["OpenAI Responses API: только по нажатию"]
    O --> AI
    AI --> U
    N --> U
...
| `ai_context` | выбранный клиент/событие → ограниченный пакет оснований | Суммы считает Python; исходные gid не передаются модели |
| `ai_service` | пакет → структурированное объяснение через OpenAI | Ключ только на сервере; проверяются формат, источники, участники и fingerprint; AI не меняет расчёт |
...
| `api` | HTTP GET → JSON, CSV, HTML; POST → AI-объяснение | Кеш по содержимому входов и конфигурации; идентификаторы в JSON — строки |
...
| `GET /api/ai/status` | Доступность настройки AI, модель и оставшиеся попытки; ключ не возвращается |
| `POST /api/clients/{gid}/ai` | Режим explain/question/brief, обязательный fingerprint, необязательный pattern_id; ответ с источниками |

Подробный контракт, состав внешнего пакета и ограничения AI описаны в [AI.md](AI.md). Основной расчёт работает без облака; только эта дополнительная функция обращается к OpenAI.
