...
После первоначальной реализации интерфейс перенесён на Next.js, а Python-ядро подключено через FastAPI. Упоминания Streamlit ниже относятся к исходному плану; актуальное разделение фронтенда и бэкенда описано в [architecture.md](architecture.md).
