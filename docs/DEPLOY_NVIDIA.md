...
Образ публикуется для `linux/amd64` и `linux/arm64`, поэтому подходит и для ARM64-инстансов NVIDIA Brev. Приложение не выполняет CUDA-вычисления, поэтому флаг `--gpus` не нужен. Данные находятся внутри образа; FastAPI доступен только внутри контейнера на `127.0.0.1:8000`, наружу опубликован Next.js на порту `3000`.
